...
def countDown(num):
    ...
    for x in range(0,num+1):
        countDownArr.append(x)

    countDownArr.reverse()    
    print(countDownArr)
...
and creates a new list containing only the values from the original list that are 
greater than its 2nd value. Print how many values this is and then return the new list. 
...
def values_greater_than_second(arr):
    ...
    for i in range(0, len(arr)):
        if (arr[i] > arr[1]) :
            greaterThanSecond.append(arr[i])
            count += 1
